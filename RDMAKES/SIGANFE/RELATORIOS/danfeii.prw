...
//** INICIO - ESPECIFICO LATICINIO SILVESTRE **//
...
//** INICIO - ESPECIFICO LATICINIO SILVESTRE **//
	private lExecjob	:= ALLTRIM(FUNNAME()) $ "WEBSERVICES"
//** FIM - ESPECIFICO LATICINIO SILVESTRE **//
	...
	If nTipo <> 1
		...
		if !lExecjob
			...
			DanfeProc(@oDanfe,,cIdEnt,,,@lExistNfe, lIsLoja)
		endif
		//** FIM - ESPECIFICO LATICINIO SILVESTRE **//
			...
			*/
			...
			If Empty(cWhere)
				...
				Else
			        ...
			        SELECT	F3_FILIAL,F3_ENTRADA,F3_NFELETR,F3_CFO,F3_FORMUL,F3_NFISCAL,F3_SERIE,F3_CLIEFOR,F3_LOJA,F3_ESPECIE,F3_DTCANC
					FROM %Table:SF3% SF3
					INNER JOIN %Table:DAI% DAI ON
					DAI.DAI_FILIAL = SF3.F3_FILIAL AND
					DAI.DAI_CLIENT = SF3.F3_CLIEFOR AND
					DAI.DAI_LOJA   = SF3.F3_LOJA AND
					DAI.DAI_NFISCA = SF3.F3_NFISCAL AND
					DAI.DAI_SERIE  = SF3.F3_SERIE															
					WHERE
					SF3.F3_FILIAL = %xFilial:SF3% AND
					SF3.F3_SERIE = %Exp:MV_PAR03% AND
					SF3.F3_NFISCAL >= %Exp:MV_PAR01% AND
					SF3.F3_NFISCAL <= %Exp:MV_PAR02% AND
					SF3.F3_DTCANC = %Exp:Space(8)% AND
					DAI.DAI_COD >= %Exp:MV_PAR07% AND
					DAI.DAI_COD <= %Exp:MV_PAR08% AND
					SF3.%notdel% AND
					DAI.%notdel%  					
					EndSql
				EndIf
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	*/		
		...
		aTamCol := {50, 126, 40, 16, 24, 14, 44, 53, 38, 36, 30, 20, 31, 24}
	endif
	//** FIM - ESPECIFICO LATICINIO SILVESTRE **//
