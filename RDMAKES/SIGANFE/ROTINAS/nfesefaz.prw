...
Private aUF     	:= {}
Private aCSTIPI 	:= {}
 //** INICIO - ESPECIFICO TRELAC **//
Private _cTipoNF   := PARAMIXB[1][1] 
//** FIM - ESPECIFICO TRELAC **//    
//Declaração de Strings
Private cFntCtrb	:= ""
				...
				*/
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						dbSelectArea("SAH")
						dbSetOrder(1)
						MsSeek(xFilial( "SAH" ) + SB1->B1_SEGUM )
		
						nPos := aScan( aEspVol, { |x| x[ 1 ] == Alltrim( SAH->AH_DESCPO ) } )
						If nPos == 0
						   nQTDVOL :=  (cAliasSD2)->D2_QTSEGUM
						   If nQTDVOL < 1
							   nQTDVOL :=  1
						   EndIf
						   
						   aAdd( aEspVol, { Alltrim( SAH->AH_DESCPO ), nQTDVOL, ( (cAliasSD2)->D2_QUANT * SB1->B1_PESO ), ( (cAliasSD2)->D2_QUANT * SB1->B1_PESBRU ) } )
						Else
						   nQTDVOL :=  (cAliasSD2)->D2_QTSEGUM
						   If nQTDVOL < 1
							   nQTDVOL :=  1
						   EndIf
					    	...
					    	aEspVol[ nPos ][ 2 ] += ( nQTDVOL				                 )
					    	aEspVol[ nPos ][ 3 ] += ( (cAliasSD2)->D2_QUANT * SB1->B1_PESO   )
					    	aEspVol[ nPos ][ 4 ] += ( (cAliasSD2)->D2_QUANT * SB1->B1_PESBRU )
						Endif                                             
						//** FIM - ESPECIFICO TRELAC **//	
						...
						EndIf
						//** INICIO - ESPECIFICO TRELAC **//				
				   		//Chama função de Mensagens Fiscais  
				  		If !"#"+ MFiscais(cAliasSD2, aDest) $ cMensFis
							cMensFis += MFiscais(cAliasSD2, aDest)
						EndIf
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			dbSelectArea("SF1")
			//** INICIO - ESPECIFICO TRELAC **//
			//--MENSAGEM NOTA FISCAL DO PRODUTOR
			If SF1->F1_TIPO = "N"	//NF ENTRADA TIPO NORMAL
				aAreaTMP := GetArea()	
				dbSelectArea("SA2")
				dbSetOrder(1)
				dbSeek ( xFilial("SA2") + SF1->F1_FORNECE + SF1->F1_LOJA )  
				If SA2->A2_X_TIPO == "P"	//PRODUTOR
					nValDEB  := fDebPROD()
					cMensCli += " # Valor Descontos (-): R$ "+Transform(nValDEB,"@E 999,999,999.99")
					cMensCli += " # Valor Funrural/Senar (-): R$ "+Transform(SF1->F1_CONTSOC+SF1->F1_VLSENAR+SF1->F1_INSS,"@E 999,999,999.99")
					cMensCli += " # Valor Liquido (=): R$ "+Transform(SF1->F1_VALBRUT-(nValDEB+SF1->F1_CONTSOC+SF1->F1_VLSENAR+SF1->F1_INSS),"@E 999,999,999.99")
				If ( SuperGetMV("MV_LTRENFP") .And. !Empty(Alltrim(SF1->F1_X_NFP)) ) .Or. !Empty(Alltrim(SF1->F1_X_NFP))
				   		cMensCli += "# NFE referente a Nota Fiscal de Produtor Rural Nº: " + ConvType(Val(SF1->F1_X_NFP),9) + " Série: " + ConvType(Val(SF1->F1_X_SNFP),3)
					EndIf
				EndIf
				RestArea(aAreaTMP)
			EndIf
...
±±ºPrograma  ³MFiscais  ºAutor  ³Joel Lipnharski     º Data ³  26/03/10   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Função de composição das mensagens fiscais na nota fiscal   º±±
±±º          ³eletronica.                                                 º±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function MFiscais(cAliasSD2,aDest) 
DEFAULT cMensFis := ""

If _cTipoNF == "1"
	
	//2-TABELA CUSTOMIZADA SZ0, Z0_FORMULA (Amarração TES/Estado/Formula)	
	_cFormSZ0 := FORMULA(POSICIONE("SZ0",1,xFilial("SZ0")+(cAliasSD2)->D2_TES+aDest[9],"Z0_FORMULA"))
	If _cFormSZ0 <> NIL
		If !"#"+ _cFormSZ0 $ cMensFis
			cMensFis += "#"+ _cFormSZ0
		EndIf
	EndIf
	
	//3-B1_X_MSGNF (Formula cadastrada no Cadastro de Produtos)
	If !Empty(SB1->B1_X_MSGNF) .And. !"#"+AllTrim(FORMULA(SB1->B1_X_MSGNF)) $ cMensFis
 		cMensFis += "#"+AllTrim(FORMULA(SB1->B1_X_MSGNF))
  	EndIf
...
/** INICIO - ESPECIFICO SILVESTRE - MENSAGEM NOTA FISCAL DO PRODUTOR **/
*--------------------------------------------*
Static Function fDebPROD()
*--------------------------------------------*
Local aAreaTMP    := GetArea()
Local cAliasTMP   := GetNextAlias()
Local cMESANO     := SubSTR(DTOS(SF1->F1_EMISSAO),1,6)
Local nVlrDED 	  := 0                                               
                                                                     
	BeginSql Alias cAliasTMP
		SELECT SUM(ZLB_VALOR) ZLB_VALOR
		FROM %Table:ZLB% ZLB
		WHERE 
		ZLB.ZLB_FILIAL = %xFilial:ZLB% AND
		ZLB.ZLB_FORNEC = %Exp:SF1->F1_FORNECE% AND
		ZLB.ZLB_LOJFOR = %Exp:SF1->F1_LOJA% AND
		SUBSTRING(ZLB.ZLB_DTVCTO,1,6) = %Exp:cMESANO% AND
		ZLB.ZLB_STATUS = %Exp:" "% AND	
		ZLB.ZLB_TIPCD  = %Exp:"D"% AND				
		ZLB.%NotDel%
	EndSql
	...
	If dbSeek ( xFilial("SA1") + SF1->F1_FORNECE + SF1->F1_LOJA )		
		...
		BeginSql Alias cAliasTMP
			...
			SE1.E1_CLIENTE 	= %Exp:SA1->A1_COD% AND
			SE1.E1_LOJA		= %Exp:SA1->A1_LOJA% AND
			SUBSTRING(SE1.E1_VENCREA,1,6) = %Exp:cMESANO% AND
			SE1.E1_SALDO    > 0 AND
			SE1.%NotDel%
		EndSql
...
/** FIM - ESPECIFICO SILVESTRE - MENSAGEM NOTA FISCAL DO PRODUTOR **/
//** FIM - ESPECIFICO TRELAC **//
