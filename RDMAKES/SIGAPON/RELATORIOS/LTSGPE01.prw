...
If Alias() <> "TMP"
	MsgBox("TMP - Parâmetros Inválidos!")
	Return .f.
Endif
nPrimer := 0
nDatImp := 0
	...
	IF LEN(aVetPon) > 0
		...
		FOR I := 1 TO LEN(aVetPon)
			IF li <= 1850
				...
				Endif
				nIncr += 030
				//removido o desenho da caixa, para que ele faça ao final: 
				...
				nIncr += 000
				oPrn:Say( Li+nIncr, nCol+0050, "Cartão Ponto", oFont6, 100 )
				nIncr += 080
				oPrn:Say( Li+nIncr, nCol+0400, SM0->M0_NOMECOM, ofont9, 100 )
				oPrn:Say( Li+nIncr, nCol+1100, "CNPJ: "+TRANSFORM(SM0->M0_CGC, "@R ##.###.###/####-##"), ofont9, 100 )
				oPrn:Say( Li+nIncr, nCol+1450, "ENDEREÇO: "+SM0->M0_ENDCOB, ofont9, 100 )
				oPrn:Say( Li+nIncr, nCol+2000, SM0->M0_CIDCOB, ofont9, 100 )
				oPrn:Box( 210, 040, 340, 2290 )
				nIncr += 60
				oPrn:Say( Li+nIncr, nCol+0000, "MATRÍCULA: "+ALLTRIM(aVetPon[I][2]), oFont2, 100 )
				oPrn:Say( Li+nIncr, nCol+0950, "FUNÇÃO: "+ALLTRIM(POSICIONE("SRJ",1,XFILIAL("SRJ")+SRA->RA_CODFUNC,"RJ_DESC")), oFont2, 100 )
				nIncr += 40
				oPrn:Say( Li+nIncr, nCol+0000, "NOME: "+ALLTRIM(SRA->RA_NOME), oFont2, 100 )
				oPrn:Say( Li+nIncr, nCol+0950, "HORÁRIO: "+ALLTRIM(POSICIONE("SR6",1,XFILIAL("SR6")+SRA->RA_TNOTRAB,"R6_DESC")), oFont2, 100 )
				nIncr += 40
				...
				oPrn:Say( Li+nIncr-40, nCol+0000, "CARTÃO PONTO", oFont2, 100 )                               
		  ...
		  //	oPrn:Say( Li+nIncr-40, nCol+1950, STR(MONTH(STOD(aVetPon[I][3])))+"/"+ALLTRIM(STR(YEAR(STOD(aVetPon[I][3])))), oFont2, 100 )
				...
				oPrn:Say( Li+nIncr, nCol+0030, "DATA", oFont2, 100 )                   
			 	oPrn:Say( Li+nIncr, nCol+0230, "MARCAÇÕES", oFont2, 100 )
				oPrn:Say( Li+nIncr, nCol+1160, "DESCRIÇÃO", oFont2, 100 )  //1200-1130
			...
			ENDDO
				...
				if ( nIncr <= 2580 )
					...
					oPrn:Box( 210, 040, 340, 2290 )
					nIncr += 60
					oPrn:Say( Li+nIncr, nCol+0000, "MATRÍCULA: "+ALLTRIM(aVetPon[1][2]), oFont2, 100 )
					oPrn:Say( Li+nIncr, nCol+0950, "FUNÇÃO: "+ALLTRIM(POSICIONE("SRJ",1,XFILIAL("SRJ")+SRA->RA_CODFUNC,"RJ_DESC")), oFont2, 100 )
					nIncr += 40
					oPrn:Say( Li+nIncr, nCol+0000, "NOME: "+ALLTRIM(SRA->RA_NOME), oFont2, 100 )
					oPrn:Say( Li+nIncr, nCol+0950, "HORÁRIO: "+ALLTRIM(POSICIONE("SR6",1,XFILIAL("SR6")+SRA->RA_TNOTRAB,"R6_DESC")), oFont2, 100 )
					...
					oPrn:Box( 410, 040, 480, 2290 )
 					oPrn:Say( Li+nIncr, nCol+0030, "DATA		MARCAÇÕES", oFont2, 100 )
					oPrn:Say( Li+nIncr, nCol+1160, "DESCRIÇÃO", oFont2, 100 )  //1200-1130
					oPrn:Say( Li+nIncr, nCol+0940, "QTDE", oFont2, 100 )
 					oPrn:Say( Li+nIncr, nCol+1850, "OBSERVACAO", oFont2, 100 )
				...
				// trará somente os novos códigos a partir da portaria 1510 implantada no mes 02/2010
		...
		oPrn:Preview()
	ELSE
		MSGALERT("NÃO EXISTEM DADOS PARA OS PARAMETROS INFORMADOS!")
	ENDIF
...
aAdd(aRegs,{cperg,"15","Exceções            ?","","","mv_chj","N",01,0,0,"C","","mv_par15","Sim","","","","","Nao","","","","","","","","","","","","","","","","","","","","",""})
...
IF LEN(ALLTRIM(SUBSTR(ALLTRIM(STR(nHora-INT(nHora))),3,2))) == 1
	...
	// controlar se os minutos são superiores a 60
	if val(cDev) > 60
		// se é superior atualizar a hora para mais 1 e a realizar diferença do minuto superior
		// com 60
...
//Função com a responsabilidade de trazer o ultimo dia do mes passado por parametro.
*-----------------------------------------------------------------*
